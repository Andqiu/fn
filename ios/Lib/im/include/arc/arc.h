...
#include "writer.hpp"
#include "globalserverconfig.hpp"
...
#define _TOKEN_PASTE_EX_(x, y) x ## y
#define LogTrace \
...
Timer *Schedule(UInt32 timeout, bool repeat, T *object, void (T::*callback)(Timer *, A1), A1 a1) {
    return ScheduleTimer(timeout, repeat, TimerCallback(object, callback, a1));
}
/**
 * 创建并启动一个定时器
 * @param timeout 超时时间
...
/**
 * @brief 初始化全局服务配置
 * @param jsonCfg json序列化配置
 * @return 是否成功，0=成功
 */
Int32 InitGlobalServerCfg(const std::string& jsonCfg);
/**
 * @brief 获取全局服务配置
 * @return 服务配置
 */
GlobalServerConfig* GetGlobalServerConfig();
/**
 * @brief 记录 IP:Port 连接耗时
 */
void SetConnTimeCost(const std::string &ip, UInt16 port, Int64 timecost);
/**
 * @brief 判断加速服务配置是否支持 udp
 */
bool isUdpAccSupported(const std::pair<std::string, UInt16>& svrInfo);
}
namespace arc {
/**
...
public:
    ScopeWatcher *Watch();
    void Unwatch(ScopeWatcher *watcher);
private:
    typedef std::list<ScopeWatcher *> ScopeWatchers;
private:
    ScopeWatchers watchers_;
};
}
using arc::Scope;
namespace arc {
...
public:
     ...
     * 是否存在加速配置
     */
    bool hasSpeedSvr() const;
    /**
     * 获取加速域名配置
     * @return pair<address, port>
     */
    std::pair<std::string, UInt16> GetSpeedSvrInfo() const;
    /**
     * 获取端口号
     * @return port 端口号
     */
...
public:
    ...
    /**
     * @brief 将hostname转成IPv4/IPv6的地址标识
     * @param hostname 待转换的hostname
     * @param outIPv4 IPv4地址
     * @param outIPv6 IPv6地址
     * @return
     */
    static Int64 FromInetAddressEx(const std::string& hostname, std::string& outIPv4, std::string& outIPv6);
    /**
     * @brief 将IPv4地址合成IPv6地址
     * @param ipv4 IPv4地址
     * @param v6PrefixAddress v6前缀地址
     * @return
     */
    static std::string SynthesisIPv6From(const std::string& ipv4, const std::string& v6PrefixAddress="");
    /**
     * @brief 从IPv6地址提取IPv6前缀
     * @param ipv6 IPv6地址
     * @return
     */
    static std::string ExtractPrefixFromIPv6(const std::string& ipv6);
    /**
     * 判断当前地址簇
     */
    static bool isIPV4();
     ...
     * @return
     */
    struct in6_addr GetIPv6InAddress() const;
    /**
     * @brief 定位用，获取相关IP信息
     * @return
     */
...
private:
    ...
    std::string ipv4_;
    std::string ipv6_;
    std::string v6Prefix_;
    std::string cvtDomain_;
    std::string speedDomain_;
    UInt16 speedPort_;
};
}
namespace arc {
...
public:
    ...
    {
        //! 是否已检测
        bool ipStackDetected;
        //! 栈类型
        IPStack::Type ipStack;
        //! v6地址前缀
        std::string ipv6Prefix;
        //! 用于检测的地址
        std::string detectedHostname;
        //! 是否强制使用ipv6
        bool isForceUseIPv6;
        //! 检测错误的返回码
        Int64 lastErrorCode;
...
public:
    ...
    /**
     * @brief 检测本地支持的IP栈
     * @param hostname 检测使用的域名，默认为空，使用@c GlobalServerConfig中的配置
     * @param directUpdate 是否直接更新底层缓存，如果为false，可以由上层手动调用 @see SetLocalIPStackInfo进行更新
     * @return 成功返回0
     */
    virtual IPStackInfo DetectLocalIPStack(const std::string& hostname="", bool directUpdate=true) = 0;
    /**
     * @brief 获取本地支持的IP栈信息
     * @return IP栈信息
     */
    virtual const std::string GetLocalIPStackInfo() const = 0;
    /**
     * @brief 更新本地协议栈信息
     * @param info @see IPStackInfo
     */
    virtual void SetLocalIPStackInfo(const IPStackInfo& info) = 0;
};
}
namespace arc {
class SocketAddress;
...
public:
     ...
     * @return
     */
    virtual std::string GetIPDomainByIP(const std::string& ip) const = 0;
    /**
     * @brief 根据服务地址，获取服务域名
     * @note 主要用于ip加速
     * @param ip IP地址
     ...
     * @return
     */
    virtual std::pair<std::string, UInt16> GetSvrInfoByAddress(const std::string& ip, UInt16 port) const = 0;
    /**
     *  @brief 获取超时次数后,需要切换网络协议的次数
     *  @return 次数
     */
    virtual int getTransNetNum() const = 0;
    /**
     * @brief 添加加速配置
     */
    virtual void AddSvrInfo(const std::string& addr, const std::string& domain, UInt16 port) = 0;
    /**
     * @brief 删除加速配置
     */
    virtual void DelSvrInfo(const std::string& addr) = 0;
}; // class GlobalServerConfig
} // namespace arc
namespace arc {
class Constants {
